...
const PersonalLoans = () => {
  ...
  return (
    <section className="relative w-full overflow-hidden rounded-[28px] bg-white">
      {/* Background image */}
      <div
        className={`relative min-h-[800px] w-full bg-cover object-fit bg-[center_40%] sm:h-[600px] lg:min-h-[620px] transition-transform duration-[3400ms] ease-out ${mounted ? "scale-100" : "scale-125"
          }`}
        style={{
          backgroundImage: "url('/loanBg.jpg')",
        }}
      >
        {/* Hero copy */}
        <div className="relative mx-auto flex max-w-5xl flex-col items-center px-5 pt-10 text-center sm:px-8 sm:pt-16 lg:px-10 lg:pt-20">
          ...
          >
            ...
            <button className="w-full group mx-auto rounded-full border border-primary py-2.5 pl-6 pr-6 text-sm font-medium text-primary transition sm:w-auto">
              Calculate Repayments
            </button>
          </div>
        </div>

        {/* Mobile Dropdown - stays absolute within the hero */}
        <div
          className={`absolute bottom-16 left-0 right-0 z-10 flex justify-center px-4 sm:hidden ${fadeUp()}`}
      ...
      {/* Desktop Pill - Fixed at top, outside the hero */}
      <nav className={`fixed w-fit left-0 right-0 bottom-15 z-50 hidden max-w-4xl mx-auto flex-wrap items-center justify-center gap-2 rounded-xl bg-white/50 p-2 shadow-lg shadow-black/5 backdrop-blur-sm sm:flex ${fadeUp()}`} style={fadeUpStyle(360)}>
        {navItems.map((item) => (
          <button
            key={item.label}
            className={
